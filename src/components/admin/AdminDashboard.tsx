...
const AdminDashboard = ({ onViewApplication }: AdminDashboardProps) => {
  ...
  const fetchApplications = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:4000/api/admin/applications?${params}`);
  ...
  const handleReview = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:4000/api/admin/applications/${selectedApp.id}`, {
        method: "PATCH",
